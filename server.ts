...
/** Escapa caracteres peligrosos para interpolacion segura dentro de strings JS en HTML */
...
const syncUserLibrary = async (hashedSteamId: string, realSteamId: string) => {
  ...
  try {
    ...
    console.log(
      `[Background Task] Estadisticas de genero guardadas: ${topGenreStats.length}`,
    );
  } catch (error) {
